      ...
      {isBurnModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
            <h2 className="text-xl font-bold mb-4 text-black">Enter Token ID to Burn</h2>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-bold text-black">
                  {burnRewardLoading ? (
                    ...
                    `${burnReward?.toFixed(3) || "0.000"} ETC`
                  )}
                </span>
              </div>
              <div className="mt-2 text-sm text-gray-600">
                <p className="text-black">Contract Balance: {contractBalance ? `${(Number(contractBalance) / 10 ** 18).toFixed(2)} ETC` : 'Loading...'}</p>
                <p className="text-black">Active NFTs: {currentTokenId && totalBurned ? `${Number(currentTokenId.toString()) - 1 - Number(totalBurned.toString())}` : 'Loading...'}</p>
              </div>
            </div>

            <input
              type="text"
              className="bg-white focus:outline-none text-black sm:text-sm focus:ring-1 rounded focus:ring-blue-600 block w-full p-2.5 pr-6 border border-gray-300 focus:border-blue-600"
